...
def parse(text):
    ...
    for m_info_clear in p_info_clear.finditer(text):
        ...
        if m_house != None:
            ...
            if len(spans) > 1:
                info["rooms"] = spans[1].strip()
            if len(spans) > 2:
                info["area"] = spans[2].strip()[:-2]
            if len(spans) > 3:
                info["direction"] = spans[3].strip()
            if len(spans) > 4:
                info["decoration"] = spans[4].strip()
            if len(spans) > 5:
                info["elevator"] = spans[5].strip()
...
def main():
    ...
    districts = ("pukou", "liuhe", "lishui", "gaochun")
